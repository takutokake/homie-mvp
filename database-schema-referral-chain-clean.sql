...
DROP POLICY IF EXISTS "Users can view own referral chain" ON referral_chain;
DROP POLICY IF EXISTS "Users can view referrals they made" ON referral_chain;
DROP POLICY IF EXISTS "Public can verify referral chain codes" ON referral_chain;
DROP POLICY IF EXISTS "Users can insert own referral chain" ON referral_chain;
...
-- Allow anonymous users to verify referral codes exist
CREATE POLICY "Public can verify referral chain codes" ON referral_chain
    FOR SELECT USING (true);

CREATE POLICY "Users can insert own referral chain" ON referral_chain
    FOR INSERT WITH CHECK (user_id = auth.uid());

-- Grant permissions
GRANT SELECT, INSERT ON referral_chain TO authenticated;
...
-- Function to get referral chain for a user
